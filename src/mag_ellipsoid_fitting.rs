...
// /// Those terms with factors of 2 are all off diagonal elements.  These contribute
// /// two terms when multiplied out (symmetric) so would need to be divided by two
// pub fn ls_ellipsoid2(sample_pts: &[Vec3; TOTAL_MAG_SAMPLE_PTS]) -> [f32; 10] {
//     //  Ax^2 + By^2 + Cz^2 +  Dxy +  Exz +  Fyz +  Gx +  Hy +  Iz = 1
//     // np.hstack performs a loop over all samples and creates
//     // a row in J for each x,y,z sample:
//     // J[ix,0] = x[ix]*x[ix]
...
//     }
//
//     let mut JT = [[0.; TOTAL_MAG_SAMPLE_PTS]; 9];
//     for c in 0..TOTAL_MAG_SAMPLE_PTS {
//         for r in 0..9 {
//             JT[r][c] = J[c][r];
//         }
//     }
...
//     for c in 0..TOTAL_MAG_SAMPLE_PTS {
//         for r in 0..9 {
//             for c_J in 0..TOTAL_MAG_SAMPLE_PTS {
//                 // JTJ[c][r] += J
//             }
//
//         }
...
// ///
// /// Algebraic form: X.T * Amat * X --> polynomial form
// pub fn poly_to_params_3d2(coeffs: &[f32; 10]) -> (Vec3, Vec3, Mat3) {
//     // #[rustfmt::skip]
//     // let amat = Mat4::new([
//     //     vec[0], vec[3]/2., vec[4]/2., vec[6]/2.,
//     //     vec[3]/2., vec[1], vec[5]/2., vec[7] / 2.,
...
//     // if printMe: print '\nAxes are\n',axes  ,'\n'
//
//     let R = eigen_vecs.try_Rrse().unwrap(); // Rrse is actually the transpose here
//     // if printMe: print '\nRotation matrix\n',R
//
//     let center = Vec3::new(center[0], center[1], center[2]);
//
//     // Our constructor is column-major. nalgebra indices are row-first.
//     // let R = Mat3::new([
//     //     R[0][0], R[1][0], R[2][0], R[0][1], R[1][1], R[2][1], R[0][2],
//     //     R[1][2], R[2][2],
//     // ]);
//     //
//     let R = Mat3::new_identity(); // todo temp; above not working yet
//
//     (center, axes, R)
// }
